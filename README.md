...
- **Whisper large-v3** via [faster-whisper](https://github.com/SYSTRAN/faster-whisper) (CTranslate2, CUDA, int8_float16)
- **Speaker diarization** via [pyannote.audio](https://github.com/pyannote/pyannote-audio) (optional, needs a free Hugging Face token)
...
Diarization uses `pyannote/speaker-diarization-3.1`, which is a gated model:

1. Create a free token at https://huggingface.co/settings/tokens
2. Accept the conditions at https://huggingface.co/pyannote/speaker-diarization-3.1 and https://huggingface.co/pyannote/segmentation-3.0
3. Put the token in `.env`: `SCRIBE_HF_TOKEN=hf_...`
...
Everything is an environment variable (or a line in `.env` at the repo root):
...
| `SCRIBE_HF_TOKEN`   | unset          | Hugging Face token for diarization         |
